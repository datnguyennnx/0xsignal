...
export class BubbleDetectionService extends Context.Tag("BubbleDetectionService")<
  BubbleDetectionService,
  {
    ...
    readonly detectPriceSpike: (price: CryptoPrice, historicalPrices?: CryptoPrice[]) => Effect.Effect<BubbleSignal | null, never>;
    ...
    readonly detectVolatilitySpike: (metrics: MarketMetrics) => Effect.Effect<BubbleSignal | null, never>;
    readonly detectExtremeDominance: (price: CryptoPrice, globalMetrics?: Record<string, unknown>) => Effect.Effect<BubbleSignal | null, never>;
...
const detectVolumeSurgeFn = (price: CryptoPrice): BubbleSignal | null => {
  ...
  const severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' =
    triggeredIndicators.length === 2 ? 'HIGH' :
    volumeToMarketCapRatio >= 0.2 ? 'CRITICAL' : 'MEDIUM';
...
const detectATHApproachFn = (price: CryptoPrice): BubbleSignal | null => {
  ...
  const severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' =
    priceToATHRatio >= 0.98 ? 'CRITICAL' :
    priceToATHRatio >= 0.95 ? 'HIGH' :
    priceToATHRatio >= 0.92 ? 'MEDIUM' : 'LOW';
...
const detectExtremeDominanceFn = (price: CryptoPrice, globalMetrics?: Record<string, unknown>): BubbleSignal | null => {
  ...
  const severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' =
    price.marketCap >= marketCapThreshold * 2 ? 'CRITICAL' :
    price.marketCap >= marketCapThreshold ? 'HIGH' : 'MEDIUM';
...
const detectVolatilitySpikeFn = (metrics: MarketMetrics): BubbleSignal | null => {
  ...
  const severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' =
    triggeredIndicators.length === 2 ? 'CRITICAL' :
    metrics.volatility >= 1.0 ? 'HIGH' : 'MEDIUM';
...
export const BubbleDetectionServiceLive = Layer.succeed(BubbleDetectionService, {
  analyzeBubble: (price, metrics) =>
    Effect.gen(function* () {
      ...
      const [priceSignal, volumeSignal, athSignal, volatilitySignal, dominanceSignal] = yield* Effect.all([
      ...
      // Collect all signals
      const signals = [priceSignal, volumeSignal, athSignal, volatilitySignal, dominanceSignal]
      ...
      const riskLevel = bubbleScore >= 80 || hasCriticalSignals ? 'EXTREME' :
                       bubbleScore >= 60 || hasHighSeveritySignals ? 'HIGH' :
  ...
  detectPriceSpike: (price) =>
    Effect.sync(() => detectPriceSpikeFn(price)),

  detectVolumeSurge: (price) =>
    Effect.sync(() => detectVolumeSurgeFn(price)),

  detectATHApproach: (price) =>
    Effect.sync(() => detectATHApproachFn(price)),

  detectVolatilitySpike: (metrics) =>
    Effect.sync(() => detectVolatilitySpikeFn(metrics)),

  detectExtremeDominance: (price, globalMetrics) =>
    Effect.sync(() => detectExtremeDominanceFn(price, globalMetrics)),

  calculateBubbleScore: (signals) =>
    Effect.sync(() => calculateBubbleScoreFn(signals))
});
