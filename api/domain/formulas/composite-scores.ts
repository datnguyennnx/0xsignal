...
export const computeMomentumScore = (
  ...
  Effect.sync(() => {
    ...
    const score = Math.round(
      (rsiScore * 0.5) + (volumeScore * 0.25) + (priceScore * 0.25)
    );
    ...
    // Generate insight
    const insight =
      signal === 'STRONG_BULLISH' ? 'Strong buying pressure with high volume' :
      signal === 'BULLISH' ? 'Positive momentum building' :
      signal === 'STRONG_BEARISH' ? 'Strong selling pressure with high volume' :
      signal === 'BEARISH' ? 'Negative momentum building' :
...
export const computeVolatilityScore = (
  ...
  Effect.sync(() => {
    ...
    const score = Math.round(
      (bbScore * 0.4) + (rangeScore * 0.4) + (athScore * 0.2)
    );
    ...
    // Generate insight
    const insight =
      regime === 'EXTREME' ? 'Extreme volatility - high risk, high reward' :
      regime === 'HIGH' ? 'High volatility - expect large price swings' :
      regime === 'NORMAL' ? 'Normal volatility - typical market conditions' :
...
export const computeMeanReversionScore = (
  ...
  Effect.sync(() => {
    ...
    // Generate insight
    const insight =
      signal === 'OVERSOLD' ? 'Price below average - potential bounce' :
      signal === 'OVERBOUGHT' ? 'Price above average - potential pullback' :
...
export const computeOverallQuality = (
  ...
  const volatilityQuality =
    volatility.regime === 'NORMAL' ? 80 :
    volatility.regime === 'HIGH' ? 60 :
    volatility.regime === 'LOW' ? 40 :
    20; // EXTREME
  ...
  return Math.round(
    (momentumQuality * 0.4) + (volatilityQuality * 0.3) + (mrQuality * 0.3)
  );
};
