...
export const cachedAnalysis = (symbol) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
    return yield* cache.analysis.get(symbol);
  });
export const cachedChartData = (symbol, interval, timeframe) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
...
export const cachedBuybackDetail = (protocol) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
    return yield* cache.buybackDetail.get(protocol);
  });
export const cachedLiquidationHeatmap = (symbol) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
    return yield* cache.liquidationHeatmap.get(symbol);
  });
export const cachedOpenInterest = (symbol) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
    return yield* cache.openInterest.get(symbol);
  });
export const cachedFundingRate = (symbol) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
...
export const invalidateAnalysis = (symbol) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
    yield* cache.analysis.invalidate(symbol);
  });
export const invalidateChartData = (symbol, interval, timeframe) =>
  Effect.gen(function* () {
    const cache = yield* CacheServiceTag;
...
//# sourceMappingURL=effect-cache.js.map
