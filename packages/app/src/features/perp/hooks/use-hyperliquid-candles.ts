/**
 * @overview Hyperliquid Candles Hook
 *
 * Fetches and manages OHLCV candlestick data with integrated WebSocket updates.
 * Supports infinite scroll (loadMore) for historical data and real-time streaming.
 *
 * @mechanism
 * - Combines REST (historical) and WebSocket (real-time) data sources
 * - Uses RAF (Request Animation Frame) batching to throttle high-frequency ticks to 60fps
 * - Implements logic to clear data on symbol change but preserve it on interval change
 *
 * @performance
 * - Deduplicates candles using a Map by timestamp (time-complexity: O(1))
 * - Debounces pagination requests to avoid API rate limiting
 * - Uses refs for mutable data to minimize React re-renders during high-frequency updates
 */
