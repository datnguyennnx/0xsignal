 ...
 *
 * Renders technical indicators (Line, Histogram, Bands) on main chart panes.
 * Uses dirty-check early exit to skip when no data changed.
 * Manages secondary panes for oscillators (RSI/MACD/etc) with cleanup.
 *
 * @mechanism
 * - Cache map (appliedCacheKeyRef) avoids redundant setData calls.
 * - Early exit skips entire effect when no cacheKey changed.
 * - Helper functions extracted for readability (remove, render).
 */
