/**
 * @overview Depth Chart Overlay Engine Hook
 *
 * It manages the rendering of dynamic elements (tooltips, crosshair, price bands) on a separate canvas layer.
 * This separation ensures that frequent overlay updates (e.g., on mouse move) don't trigger expensive re-renders of the main depth area.
 *
 * @mechanism
 * - uses a ResizeObserver to sync the overlay canvas with the container size.
 * - implements high-frequency drawing of vertical guides, price markers, and "mirror" points.
 * - handles theme-aware colors for a consistent look across light and dark modes.
 */
