/**
 * @overview Depth Chart Canvas Engine
 *
 * Implements the core rendering logic for the depth chart using HTML5 Canvas.
 * Handles grid drawing, axes, and the filled area paths for bids and asks.
 *
 * @mechanism
 * - uses a throttled update system (UPDATE_THROTTLE_MS) to prevent over-rendering during high-frequency L2 updates.
 * - implements hash-based comparison to avoid re-drawing if the depth data hasn't changed.
 * - handles DPI scaling (window.devicePixelRatio) for crisp rendering on high-res displays.
 */
