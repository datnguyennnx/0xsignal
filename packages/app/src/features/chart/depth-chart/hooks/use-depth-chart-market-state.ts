/**
 * @overview Depth Chart Market State Hook
 *
 * Tracks the "stable" center price and mid-price for the depth chart.
 * Implements a "deadband" and "reconciliation ramp" to prevent the chart from jumping on every micro-tick.
 *
 * @mechanism
 * - computes a live mid-price from the best bid/ask.
 * - uses a deadband (resolveCenterRampParams) to determine if a re-center is necessary.
 * - implements a "slew-rate" cap (maxStepPerUpdate) to smoothly transition the chart center.
 */
