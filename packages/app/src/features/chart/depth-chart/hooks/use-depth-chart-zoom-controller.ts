/**
 * @overview Depth Chart Zoom Controller Hook
 *
 * Manages the interactive zooming (pinch-to-zoom, mouse wheel) for the depth chart.
 * It handles the state of the "desired" half-span and provides mechanisms for interaction tracking.
 *
 * @mechanism
 * - uses WheelEvents and TouchEvents to calculate zoom factors.
 * - implements an interaction idle timeout to prevent UI flickering during rapid updates.
 * - tracks isInteracting state to inform other hooks (like canonical-frame) to pause automatic reconciliation.
 */
