/**
 * @overview Depth Chart Hover & Resolution Model
 *
 * Implements the logic to map canvas interaction points back to market data (price, size).
 * Features "sticky" hover behavior and support for spread gap detection.
 *
 * @mechanism
 * - uses a binary-search-like approach to find the nearest price level to a canvas coordinate.
 * - implements "sticky bands" (isWithinBandWithPad) to keep the tooltip stable when the mouse moves slightly within a price level.
 * - calculates "mirror" points (resolveDepthSideSnapshotAtPrice) for symmetric depth analysis.
 */
