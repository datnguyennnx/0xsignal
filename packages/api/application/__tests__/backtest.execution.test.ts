...
describe("Backtest Execution Flow", () => {
  it("should trigger background worker and update row status when backtest is started", async () => {
    ...
    const mockRepo = {
      ...
      updateRunStatus: vi.fn().mockImplementation((id, status) => Promise.resolve()),
    ...
    const backtestService = makeBacktestService(mockRepo as any);

    const runId = "test-run-123";
    const effect = backtestService.createBacktestRun({
      id: runId,
      strategy_version_id: "strat-1",
      dataset_snapshot_id: "data-1",
      status: "pending",
